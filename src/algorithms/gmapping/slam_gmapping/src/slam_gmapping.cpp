...
void SlamGmapping::startLiveSlam() 
{
    ...
    sst_ = this->create_publisher<nav_msgs::msg::OccupancyGrid>("map", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable()); // /map topic publisher 객체
    sstm_ = this->create_publisher<nav_msgs::msg::MapMetaData>("map_metadata", rclcpp::QoS(rclcpp::KeepLast(1)).transient_local().reliable());
    scan_filter_sub_ = std::make_shared<message_filters::Subscriber<sensor_msgs::msg::LaserScan>>(this, "scan", rclcpp::SensorDataQoS().get_rmw_qos_profile()); // /scan topic subscriber 객체
